...
class _EditCoQState extends State<EditCoQ> {
  ...
  void initState() {
    ...
    _subjectController = TextEditingController(text: widget.coq.activityName);
    ...
    _lecturerController = TextEditingController(text: widget.coq.lecturerName);

    if (widget.coq.date != null) {
      _selectedDate = widget.coq.date!.toDate();
    }

    if (widget.coq.time != null) {
      _selectedTime = TimeOfDay.fromDateTime(widget.coq.time!.toDate());
    }
  }
  ...
  void _updateForm() async {
    ...
    final ModuleCoQ updatedCoQ = ModuleCoQ(
      coqId: widget.coq.coqId,
      ...
      booked: widget.coq.booked,
      status: widget.coq.status,
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SingleChildScrollView(
        padding: const EdgeInsets.only(top: 52, bottom: 35),
        child: Center(
          child: Container(
            width: 296,
      ...
      builder: (context) => const AlertDialog(
        content: Column(
          ...
          children: [
            ...
            Text('Co-Q Activity has been updated successfully.'),
          ],
        ),
      ),
    );
