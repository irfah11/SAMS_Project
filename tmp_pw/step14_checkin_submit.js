...
(async () => {
  ...
  try {
    ...
    // Toggle GPS switch
    await page.mouse.click(1209, 334);
    await page.waitForTimeout(500);
    await enableSemantics(page);
    await page.screenshot({ path: 'screens/18_gps_enabled.png' });

    // Type the code
    const input = await page.$('input[aria-label="Enter 6-character code (e.g. QWE123)"]');
    await input.click();
    await input.fill('EW5Y6I');
    await page.waitForTimeout(500);
    await page.screenshot({ path: 'screens/19_code_entered.png' });

    // Click Enter
    await enableSemantics(page);
    btns = await getButtons(page);
    const enterBtn = btns.find(b => b.text === 'Enter');
    console.log('Enter button:', JSON.stringify(enterBtn));
    await page.mouse.click(enterBtn.x, enterBtn.y);
    await page.waitForTimeout(2000);
    await enableSemantics(page);
    await page.screenshot({ path: 'screens/20_checkin_result.png' });
    ...
    console.log('Result screen text:', JSON.stringify(full, null, 1));
  } catch (e) {
    console.error('ERROR:', e.message);
    await page.screenshot({ path: 'screens/ERROR_step14.png' });
  } finally {
    await browser.close();
  }
})();
